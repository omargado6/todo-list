...
import { doneTask } from '../rtk/Slices/DoneTaskSlice';
            ...
            <ul className="py-2 max-[355px]:w-max">
                {Todolists.map((Element) => (
                    <li key={Math.random()} className="py-4 px-1 flex gap-1 w-full   shadow-3xl rounded-lg hover:-translate-y-1 duration-300">
                        <text className="text-sky3 font-serif max-sm:text-xs pl-1 text-left w-8/12">{Element.title}</text>
